...
#define GPIO_GSE_SDA_PIN GPIO60
#define GPIO_GSE_SCL_PIN GPIO63
...
static int  MXC400X_RxData(struct i2c_client *client, s16 data[MXC400X_AXES_NUM])
{
    ...
    err = mxc400x_i2c_read_block(client, addr, buf, MXC400X_DATA_LEN);
    if(err)
    {
      GSE_ERR("error: %d\n", err);
    }
...
{     
	...
	err = mxc400x_i2c_read_block(client, addr, &temp, 1);
	if(err)
	{
		GSE_ERR("error: %d\n", err);
	}
...
static int MXC400X_ReadDant(struct i2c_client *client, int dant[MXC400X_AXES_NUM+1])
{
	...
	err = mxc400x_i2c_read_block(client, addr, buf, MXC400X_DATA_LEN+1);
	if(err)
	{
		GSE_ERR("error: %d\n", err);
	}
...
static int MXC400X_ReadOffset(struct i2c_client *client, s16 ofs[MXC400X_AXES_NUM])
{    
	int err;
	err = 0;
...
#else
	divisor = obj->reso->sensitivity/lsb;//modified
...
//static hwm_sensor_data* gsensor_data;
#if 0
	 ...
	 switch (command)
	 {
		 ...
		 case SENSOR_GET_DATA:
			 ...
			 else
			 {
				 ...
				 gsensor_data->values[2] = cali_sensor_data;
				 mutex_unlock(&mxc400x_mutex);
...
static const struct file_operations mxc400x_fops = {
		 //.owner = THIS_MODULE,
		 .open = mxc400x_open,
